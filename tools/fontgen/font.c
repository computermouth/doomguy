...
ww_sprite_t * sprite_from_string(char * input){
	...
	for(int i = 0; i < len; i++){
		...
		for(int j = 1; j < fc.lines + 1; j++){
			...
			//~ ref.arrays[(total_lines + j) * 2 - 2][0] = (ref.arrays[(total_lines + j) * 2 - 2][0] > 0) ? 8 * ref.arrays[(total_lines + j) * 2 - 2][0] - 1: 0;
			//~ ref.arrays[(total_lines + j) * 2 - 2][1] = (ref.arrays[(total_lines + j) * 2 - 2][1] > 0) ? 8 * ref.arrays[(total_lines + j) * 2 - 2][1] - 1: 0;
			//~ ref.arrays[(total_lines + j) * 2 - 2][2] = (ref.arrays[(total_lines + j) * 2 - 2][2] > 0) ? 8 * ref.arrays[(total_lines + j) * 2 - 2][2] - 1: 0;
			//~ ref.arrays[(total_lines + j) * 2 - 2][3] = (ref.arrays[(total_lines + j) * 2 - 2][3] > 0) ? 8 * ref.arrays[(total_lines + j) * 2 - 2][3] - 1: 0;
			
			//~ ref.arrays[(total_lines + j) * 2 - 1][0] = (ref.arrays[(total_lines + j) * 2 - 1][0] > 0) ? 8 * ref.arrays[(total_lines + j) * 2 - 1][0] - 1: 0;
			//~ ref.arrays[(total_lines + j) * 2 - 1][1] = (ref.arrays[(total_lines + j) * 2 - 1][1] > 0) ? 8 * ref.arrays[(total_lines + j) * 2 - 1][1] - 1: 0;
			//~ ref.arrays[(total_lines + j) * 2 - 1][2] = (ref.arrays[(total_lines + j) * 2 - 1][2] > 0) ? 8 * ref.arrays[(total_lines + j) * 2 - 1][2] - 1: 0;
			//~ ref.arrays[(total_lines + j) * 2 - 1][3] = (ref.arrays[(total_lines + j) * 2 - 1][3] > 0) ? 8 * ref.arrays[(total_lines + j) * 2 - 1][3] - 1: 0;			
			
			printf("\t\t\t(short[]){ %d, %d, %d, %d },\n",
				8 * ref.arrays[(total_lines + j) * 2 - 2][0],
				8 * ref.arrays[(total_lines + j) * 2 - 2][1],
				8 * ref.arrays[(total_lines + j) * 2 - 2][2],
				8 * ref.arrays[(total_lines + j) * 2 - 2][3]
			);
			printf("\t\t\t(short[]){ %d, %d, %d, %d }",
				8 * ref.arrays[(total_lines + j) * 2 - 1][0],
				8 * ref.arrays[(total_lines + j) * 2 - 1][1],
				8 * ref.arrays[(total_lines + j) * 2 - 1][2],
				8 * ref.arrays[(total_lines + j) * 2 - 1][3]
			);
