...
void ww_scale_polygon(ww_polygon_t * poly){
	...
	if( poly->ratio   != window_p->ww_ratio || 
		...
		poly->s_miny = poly->scaled_y[0];
		poly->s_maxy = poly->scaled_y[0];
		
		poly->s_minx = poly->scaled_x[0];
		poly->s_maxx = poly->scaled_x[0];
		...
		for(int i = 0; i < poly->count; i++){
			...
			if (poly->scaled_x[i] < poly->s_minx){
				poly->minx   = poly->x[i];
				poly->s_minx = poly->scaled_x[i];
			} else if (poly->scaled_x[i] > poly->s_maxx){
				poly->maxx   = poly->x[i];
				poly->s_maxx = poly->scaled_x[i];
			}
			
			if (poly->scaled_y[i] < poly->s_miny){
				poly->miny   = poly->y[i];
				poly->s_miny = poly->scaled_y[i];
			} else if (poly->scaled_y[i] > poly->s_maxy){
				poly->maxy   = poly->y[i];
				poly->s_maxy = poly->scaled_y[i];
			}
		}
...
void ww_scale_frame(ww_frame_t * frame){
	...
	int minx = frame->polys[0].minx;
	int maxx = frame->polys[0].maxx;
	
	int miny = frame->polys[0].miny;
	int maxy = frame->polys[0].maxy;
	...
	if( (maxx - minx) > parent->width)
		parent->width = (maxx - minx) * parent->scale;
	
	if( (maxy - miny) > parent->height)
		parent->height = (maxy - miny) * parent->scale;
}
...
int ww_draw_polygon(ww_polygon_t * poly){
	...
	return ww_draw_raw_polygon(poly->scaled_x, poly->scaled_y, poly->count, poly->s_miny, poly->s_maxy, poly->color);
}
